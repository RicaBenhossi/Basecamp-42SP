...
/*   Updated: 2021/06/19 04:25:14 by luricard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		main(void)
{
	...
	while (offset < 10)
	{
		ft_strncmp_test(s1, s1, offset);
		offset++;
	}
	offset = 0;
	printf("[5] Testing strings \"%s\" and \"%s\" (different lengths)\n", s1, s4);
	while (offset < 10)
	{
	...
	offset = 0;
	printf("[6] Testing strings \"%s\" and \"%s\" (size 0)\n", s1, s4);
	ft_strncmp_test(s1, s4, offset);
	return (0);
}
